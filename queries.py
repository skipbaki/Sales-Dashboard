...
def monthly_sales():
    if USE_SQLITE:
        ...
        sql = """ SELECT TO_CHAR(TO_DATE(order_date, 'YYYY-MM-DD'), 'YYYY-MM') AS month, 
               SUM(oi.quantity * oi.unit_price) AS monthly_sales
        FROM orders o
        JOIN order_items oi ON o.order_id = oi.order_id
...
def top_products(limit=5):
    sql = f""" SELECT p.name AS product, SUM(oi.quantity * oi.unit_price) AS revenue
    FROM products p
...
def top_customers(limit=5):
    sql = f""" SELECT c.name AS customer, SUM(oi.quantity * oi.unit_price) AS spending
    FROM customers c
...
if __name__ == '__main__':
    ...
    print("KPI CSV saved to outputs/")
